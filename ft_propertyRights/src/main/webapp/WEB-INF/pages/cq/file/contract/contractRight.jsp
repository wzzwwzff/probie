...
<div class="context">
    <div class="data_table_height_auto" style="padding: 0px;">
        <ul id="myTab" class="nav nav-tabs" style="padding: 0px;">
            <li class="active"><a href="#home" onclick="hideprofile();" data-toggle="tab"
                                  aria-expanded="true">正常上传资料</a></li>
        ...
        <div id="myTabContent" class="tab-content">
            <div class="tab-pane fade active in" style="margin-top: 9px" id="home">
                <table>
                    <tr>
                        <td style="width: 450px;vertical-align: top;" class="text-left">
                    ...
                    <ul class="docs-pictures clearfix">
                        ...
                        <li style="width: 20%;">
                            <a href="javascript:void(0);"
                               ...
                               class="thumbnail no-margin"
                               data-original-title="" title="点击查看">
                                &lt;%&ndash;<input type="checkbox" cqCheckStatus="<%=fileInfo.getOneFileStatus()%>"&ndash;%&gt;
                                &lt;%&ndash;sjCheckStatus="<%=fileInfo.getSjFileStatus()%>" name="imgCheck"&ndash;%&gt;
                                &lt;%&ndash;value="<%=fileInfo.getUuid()%>" id="<%=fileInfo.getFileName()%>"&ndash;%&gt;
                                &lt;%&ndash;class="check_img myInput"/>&ndash;%&gt;
                                <c:if test="${fn:length(fileType) eq 4}">
                                    <img data-original="<%=s_filePath%>"
                                         src="<%=s_filePath%>" id="<%=fileInfo.getFileName()%>"
                                ...
                                <c:if test="${fn:length(fileType) eq 6}">
                                    <p style="height: 160px;text-align: center;vertical-align: middle;line-height: 45px;"
                                       readonly="true">
                                        <span style="font-weight: bold;text-align: center;font-size: 20px;"><%=fileInfo.getFileName() + "." + fileInfo.getDeleteState()%></span><br/>
                                        <span style="color: red;font-weight: bold;">（不支持在线预览，请点击“下载”查看）</span>
                                    </p>
                                </c:if>
                            </a>
...
//                    var prefix = $(this).attr("name");
    ...
    function batchDownFile(ids) {
        location.href = "downContractFile?ids=" + ids;
    }
    ...
    function deleteFileInfo(uuid, deleteYn) {
        if (deleteYn == 2) {
            ...
            if (confirm("确定要删除此图片吗?")) {
                $.get("/cq/file/project/deleteProjectInfo", {uuid: uuid, _date: new Date().getTime()}, function (data) {
                    if (data.code == "0000") {
                        commonReload();
                    } else {
                        alert(data.message);
