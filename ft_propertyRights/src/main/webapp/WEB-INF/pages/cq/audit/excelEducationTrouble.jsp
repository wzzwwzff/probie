...
<table width="1290" border="0" cellpadding="0" cellspacing="0"
    ...
    <tr height="56.67" style='height:34.00pt;mso-height-source:userset;mso-height-alt:680;'>
        <td class="xl67" height="56.67" width="1290" colspan="30"
    ...
    <tr height="41.67" style='height:25.00pt;mso-height-source:userset;mso-height-alt:500;'>
        ...
        <td class="xl70" x:str>学历审核不通过原因</td>
    ...
    <c:forEach items="${pageInfo.result}" var="family" varStatus="status">
        <tr height="41.67" style='height:25.00pt;mso-height-source:userset;mso-height-alt:500;'>
            <td class="xl70" x:str height="41.67" style='height:25.00pt;'>${status.count}</td>
            <td class="xl70" x:str><tags:dataDict dataDict="项目名称" value="${family.projectName}"/></td>
            <td class="xl70" x:str><tags:dataDict dataDict="家庭类型" value="${family.familyType}"/></td>
            <td class="xl70" x:str>${family.familyCode}</td>
            <td class="xl70" x:str>${family.name}</td>
            <td class="xl70" x:str>${family.age}</td>
            <td class="xl70" x:str><tags:dataDict dataDict="证件类型" value="${family.cardType}"/></td>
            <td class="xl70" x:str>${family.idCard}</td>
            <td class="xl70" x:str><tags:dataDict dataDict="婚姻状况" value="${family.marryStatus}"/></td>
            <td class="xl70" x:str>${family.address}</td>
            <td class="xl70" x:str><tags:dataDict dataDict="工作地（社保缴纳地）" value="${family.workUnit}"/></td>
            <td class="xl70" x:str>${family.registerCode}</td>
            <td class="xl70" x:str><tags:dataDict dataDict="是否" value="${family.thisArea}"/></td>
            <td class="xl71" x:str><fmt:formatDate value="${family.applyDate}" pattern="yyyy-MM-dd" /></td>
            <%--<td class="xl70" x:str>${family.exSpouseName}</td>
            <td class="xl70" x:str><tags:dataDict dataDict="证件类型" value="${family.exCardType}"/></td>
            <td class="xl70" x:str>${family.exIdCard}</td>
            <td class="xl71" x:str><fmt:formatDate value="${family.divorceDate}" pattern="yyyy-MM-dd" /></td>--%>
            <td class="xl70" x:str>${family.contactName}</td>
            <td class="xl70" x:str>${family.phone}</td>
            <td class="xl70" x:str>${family.currentAddres}</td>
            <td class="xl70" x:str>${family.postCode}</td>
            <td class="xl70" x:str>${family.memo}</td>
            <%--<td class="xl70" x:str>${family.shakingBatch}</td>
            <td class="xl70" x:str>${family.shakingCode}</td>
            <td class="xl70" x:str>${family.shakingGroup}</td>
            <td class="xl70" x:str>${family.shakingResult}</td>--%>
            <td class="xl70" x:str><tags:dataDict dataDict="是否" value="${family.firstOrNot}"/></td>
            <td class="xl70" x:str><tags:dataDict dataDict="是否" value="${family.continuity}"/></td>
            <td class="xl70" x:str><tags:dataDict dataDict="学历层次" value="${family.education}"/></td>
            <td class="xl70" x:str><tags:dataDict dataDict="学习形式" value="${family.learning}"/></td>
            <td class="xl70" x:str><tags:dataDict dataDict="是否" value="${family.thisPark}"/></td>
            <td class="xl70" x:str>${family.educationCode}</td>
            <td class="xl70" x:str>${family.educationTrouble}</td>
            <td class="xl70" x:str>${family.job}</td>
            <td class="xl70" x:str>${family.unit}</td>
            <td class="xl70" x:str>${family.unitPerson}</td>
            <td class="xl70" x:str>${family.unitPhone}</td>
        </tr>
    </c:forEach>
