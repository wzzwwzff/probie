...
public class FamilyService extends HibernateDao<Family>{
    ...
    public PageInfo<Family> listForHand(PageInfo<Family> pageInfo){
        StringBuffer hql = new StringBuffer("from Family order by id");
        return this.list(pageInfo,hql.toString());
    }
    ...
    /**
     * 学历验证码不为空  已上报的且审核状态为不通过的
     *@param pageInfo
     * @return
     */
    public PageInfo<Family> listUnEducationCheck(PageInfo<Family> pageInfo, HttpServletRequest request){
        StringBuffer hql = new StringBuffer("from Family where dataStatus = 2 and educationCode is not null and educationCode <> '' and educationStatus = 3 order by id");
        return this.list(pageInfo,hql.toString());
    }
    /**
     * 学历验证码不为空  已上报的
    ...
    public PageInfo<Family> listEducationCheck(PageInfo<Family> pageInfo, HttpServletRequest request){
        StringBuffer hql = new StringBuffer("from Family where dataStatus = 2 and educationCode is not null and educationCode <> '' and educationStatus = 1 order by id");
        return this.list(pageInfo,hql.toString());
    }
