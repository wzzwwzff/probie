...
<html lang="zh_CN">
<%
    ProjectFile fileInfoList = (ProjectFile) request.getAttribute("projectFile");
%>
<head>
    <meta charset="UTF-8">
    <title>创建文件夹</title>
...
<div class="content">
        <form class="form-horizontal" id="inputForm" action="contractFileSave?proFileId=${projectFile.id}&<tags:searchParams></tags:searchParams>" method="post" autocomplete="off">
            <input type="hidden" name="id" value="${projectFile.id}">
            <input type="hidden" name="unitId" value="${contractId}">
