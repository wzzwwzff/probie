...
<div class="title">
    <h4 style="color: #0d88cc">本市户籍家庭信息列表</h4>
</div>
<div class="content">
        <div class="search_content resizeLayout">
                <form class="form-inline" role="form">
                    ...
                    <a href="form" class="btn btn-primary btn-sm"><i class="glyphicon glyphicon-plus"></i>登记本市户籍家庭信息</a>
                    <a href="excelForm?familyType=1" class="btn btn-primary btn-sm"><i
                            class="fa fa-file-excel-o"></i> 导出家庭信息台账</a>&nbsp;&nbsp;
                    <a href="/cq/compareInfo/innerExcelIndex?familyType=1" class="btn btn-primary btn-sm"><i class="fa fa-file-excel-o"></i>批量导入数据</a>
            ...
            <tr>
                ...
                <td style="text-align: center;width: 20%">
                    <c:if test="${family.dataStatus < 2}">
                        ...
                        <a href='checkForm?familyId=${family.id}&<tags:searchParams></tags:searchParams>' style="color: #00B83F"
...
<script>
    ...
    function cancelCheck(familyId) {
        if (confirm("确认撤销复核上报吗？\n撤销后数据将可以修改！")) {
            $.get("cancelCheck", {familyId: familyId}, function (data) {
                if (data.code == "0000") {
                    location.reload();
                } else {
                    alert(data.message);
