...
public class StatisticsService extends HibernateDao<Family> {
    ...
    public Map<String,Map<String,Map>> houseMap() {
        String sql = "select h.buildNum,h.houseType," +
                ...
                " from house h group by h.buildNum,h.houseType " +
                " ORDER BY h.buildNum+0";
    ...
    public List houseStatistics() {
        String sql = "select sum(case when housetype = 1 then 1 else 0 end)," +
                "sum(case when houseType = 1 then buildArea else 0 end)," +//全部房源数量
                "sum(case when houseType = 1 then sjBuildArea else 0 end)," +//全部预测面积
                "sum(case when houseType = 3 then 1 else 0 end)," +//全部实际面积
                "sum(case when houseType = 3 then buildArea else 0 end)," +//已选房源数量
                "sum(case when houseType = 3 then sjBuildArea else 0 end)," +//已选房源预测面积
                "sum(case when houseType = 4 then 1 else 0 end)," +//已选房源实际面积
                "sum(case when houseType = 4 then buildArea else 0 end)," +//剩余房源数量
                "sum(case when houseType = 4 then sjBuildArea else 0 end)," +//剩余房源预测面积
                "sum(case when houseType = 1 and houseStatus > 1 then 1 else 0 end)," +//剩余房源实际面积
                "sum(case when houseType = 1 and houseStatus > 1 then buildArea else 0 end)," +//剩余房源实际面积
                "sum(case when houseType = 1 and houseStatus > 1 then sjBuildArea else 0 end)," +//剩余房源实际面积
                "sum(case when houseType = 3 and houseStatus > 1 then 1 else 0 end)," +//剩余房源实际面积
                "sum(case when houseType = 3 and houseStatus > 1 then buildArea else 0 end)," +//剩余房源实际面积
                "sum(case when houseType = 3 and houseStatus > 1 then sjBuildArea else 0 end)," +//剩余房源实际面积
                "sum(case when houseType = 4 and houseStatus > 1 then 1 else 0 end)," +//剩余房源实际面积
                "sum(case when houseType = 4 and houseStatus > 1 then buildArea else 0 end)," +//剩余房源实际面积
                "sum(case when houseType = 4 and houseStatus > 1 then sjBuildArea else 0 end)," +//剩余房源实际面积
                "sum(case when houseType = 1 and houseStatus = 1 then 1 else 0 end)," +//剩余房源实际面积
                "sum(case when houseType = 1 and houseStatus = 1 then buildArea else 0 end)," +//剩余房源实际面积
                "sum(case when houseType = 1 and houseStatus = 1 then sjBuildArea else 0 end)," +//剩余房源实际面积
                "sum(case when houseType = 3 and houseStatus = 1 then 1 else 0 end)," +//剩余房源实际面积
    ...
    public PageInfo<Family> getListForCompositeSearch(PageInfo<Family> pageInfo, HttpServletRequest request) {
        ...
        while (paramsArray.hasMoreElements()) {
            ...
            /*if (name.endsWith("TimeDu")) {
                String dateArray = ParamUtils.getString(request, name, "");
                if(Strings.isNotEmpty(dateArray) && !dateArray.equals(",")){
                    ...
                    if (Strings.isNotEmpty(startDate) && Strings.isNotEmpty(endDate)) {
                        String newName = name.substring(name.indexOf("_") + 1);
                        String columnName = newName.substring(0, newName.indexOf("_"));
                        hql.append(" and convert( " + columnName + ",char(10))" + " between '" +  startDate + "' and '" + endDate +"'");
                    }
                }
            }*/
        }
