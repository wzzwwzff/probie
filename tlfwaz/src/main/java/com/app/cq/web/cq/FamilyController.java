...
public class FamilyController {
    ...
    @ResponseBody
    public Map handPcNum(HttpServletRequest request){
        Map map = this.familyService.setPhoneBatch(request);
        return map;
    }
    ...
    public void phoneHandList(HttpServletRequest request, ModelMap modelMap){
        ...
        pageInfo = areaService.getListArea(pageInfo);
        modelMap.addAttribute("pageInfo",pageInfo);
    }
    ...
    public void printHq(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        Family family = familyService.familyForm(familyId);
        List<DataDict> contextList = this.dataDictService.getDataDictList("文本单据");
        List<DataDict> batchDateList = this.dataDictService.getDataDictList("批次时间段");
        String batchDate = "";
        for (DataDict dataDict:batchDateList) {
            if (family.getBatchDate().toString().equals(dataDict.getAttributeValue())){
                batchDate = dataDict.getAttributeName();
            }
        }
    ...
    public String specialSave(HttpServletRequest request, ModelMap modelMap) {
        ...
        Servlets.bind(request,family);
        if (family.getIsSpecialFamily() == SpecialStatus.NO.getIndex()){
            family.setSpecialReason(null);
        }
        family.setSpecialOperationDate(new Date());
        family.setSpecialOperationPerson(GetCurrentUser.getLoginUser(request).getRealName());
        this.familyService.save(family);
