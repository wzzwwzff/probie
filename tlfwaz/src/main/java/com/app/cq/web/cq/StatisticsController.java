...
public class StatisticsController {
    ...
    public void pytjStatistics(ModelMap modelMap){
        Map<String,Map> signMap = statisticsService.pyprocessMap();
        Map<String,Map> xfMap = statisticsService.pyprocessMap1();
        modelMap.addAttribute("signMap",signMap);
        modelMap.addAttribute("xfMap",xfMap);
    }
...
//        }
        Map<String,Map<String,Map>> map=statisticsService.userHandleMap();
        ...
        if (pageInfo.getTotalCount() > 0) {
            ...
            for (Family family : (List<Family>) pageInfo.getResult()) {
                ...
                for (int i = 0; i < nameList.size(); i++) {
                    ...
                    }else if (property.equals("f_signDate") || property.equals("f_moveBackDate") || property.equals("f_xfDate") || property.equals("f_xyPrintDate") || property.equals("f_jsDate") || property.equals("f_signNumDate")) {
                        if (value == null) {
                            value = "";
                        }else{
                            try {
