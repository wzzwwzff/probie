...
<table width="1290" border="0" cellpadding="0" cellspacing="0"
    ...
    <tr height="41.67" style='height:25.00pt;mso-height-source:userset;mso-height-alt:500;'>
        <td class="xl70" rowspan="2" height="41.67" style='height:25.00pt;' x:str>序号</td>
        <td class="xl70" rowspan="2" x:str>项目名称</td>
        <td class="xl70" rowspan="2" x:str>地块</td>
        <td class="xl70" rowspan="2" x:str>档案编号</td>
        <td class="xl70" rowspan="2" x:str>姓名</td>
        <td class="xl70" rowspan="2" x:str>联系电话</td>
        <td class="xl70" rowspan="2" x:str>选房批次</td>
        <td class="xl70" rowspan="2" x:str>选房时间</td>
        <td class="xl70" rowspan="2" x:str>选房时间段</td>
        <td class="xl70" colspan="4" x:str>选房情况</td>
    </tr>
    <tr height="41.67" style='height:25.00pt;mso-height-source:userset;mso-height-alt:500;'>
        <td class="xl70" x:str>楼号</td>
        <td class="xl70" x:str>单元号</td>
        <td class="xl70" x:str>房号</td>
        <td class="xl70" x:str>预测面积</td>
    </tr>
    <c:forEach items="${pageInfo.result}" var="family" varStatus="status">
        <c:set var="houseListSize" value="${family.houseList.size()}" />
        <c:set var="size" value="${houseListSize}" />
        <tr height="41.67" style='height:25.00pt;mso-height-source:userset;mso-height-alt:500;'>
            <td class="xl70" rowspan="${size}" x:str height="41.67" style='height:25.00pt;'>${status.count}</td>
            <td class="xl70" rowspan="${size}" x:str>${family.project.projectName}</td>
            <td class="xl70" rowspan="${size}" x:str>${family.area.areaName}</td>
            <td class="xl70" rowspan="${size}" x:str>${family.familyCode}</td>
            <td class="xl70" rowspan="${size}" x:str>${family.name}</td>
            <td class="xl70" rowspan="${size}" x:str>${family.phone}</td>
            <td class="xl70" rowspan="${size}" x:str>${family.xfBatch}</td>
            <td class="xl70" rowspan="${size}" x:str><fmt:formatDate value="${family.xyXfDate}" pattern="yyyy-MM-dd" /></td>
            <td class="xl70" rowspan="${size}" x:str><tags:dataDict dataDict="批次时间段" value="${family.batchDate}"/></td>
            <c:forEach items="${family.houseList}" var="house" varStatus="status1">
                <td class="xl70" x:str>${house.buildNum}</td>
                <td class="xl70" x:str>${house.unitNum}</td>
                <td class="xl70" x:str>${house.houseNum}</td>
                <td class="xl70" x:str>${house.buildArea}</td>
                </tr>
            </c:forEach>
    </c:forEach>
