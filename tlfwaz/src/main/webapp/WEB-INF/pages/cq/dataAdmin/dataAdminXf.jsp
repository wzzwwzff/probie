...
<div class="content">
    ...
    <table class="table table-bordered table-hover table-striped">
        ...
        <tr>
            ...
            <th rowspan="2" colspan="2">签约业务办理</th>
        </tr>
        <tr>
            <th>家庭编号</th>
            <th>被征收人</th>
        ...
        <c:forEach varStatus="status1" var="family" items="${pageInfo.result}">
            <c:forEach varStatus="status2" var="house" items="${houseListMap[family.id]}">
                ...
                <td><fmt:formatDate value="${house.chooseDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                    <c:if test="${status2.index eq 0}">
                        <td rowspan="${lenth}">
                                <a href="dataAdminChooseHouse?familyId=${family.id}&searchParams=<tags:searchParams/>"><img
                                        src="/static/image/manager.gif"/></a>
                            </td>
                            <td rowspan="${lenth}">
                                <a href="/cq/chooseHouse/printAgreement?familyId=${family.id}"><i class="fa fa-edit"></i>签约业务办理</a>
                            </td>
                        <td rowspan="${lenth}">
                            <a href="/cq/chooseHouse/printChooseHouseResult?familyId=${family.id}"><i class="fa fa-print"></i>打印选房确认单</a>
                        </td>
                    </c:if>
                </tr>
            </c:forEach>
