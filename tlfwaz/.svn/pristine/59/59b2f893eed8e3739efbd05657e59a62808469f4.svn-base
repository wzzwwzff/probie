...
//                                str2 = dataDict.getAttributeName();
            ...
            <td <%=click%> height="40" width="<%=width %>%" bgcolor="<%=bgColor%>" class='msg_tk'
                style="text-align:center;cursor: pointer;<%if(j==maxSetNum-1){%>border-left:#999999 0px solid;<%}%>
...
<script>
   /* $(function () {
        /!*$(".msg_tk").bind("click", function () {*!/
        $("input[name='hId']").bind("click", function () {
            var num = $(this).attr("title"); //1号楼单元1101房
            var a = $(this).attr("op"); //备注
            var houseId=$(this).attr("value");//房源id回显弹框信息
