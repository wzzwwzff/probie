    ...
    <div>
        <center>
            <div class="input-group" style="width: 650px;margin-top: 120px;">
                <input type="text" name="familyCode" id="familyCode" placeholder="请输入选房顺序号" size="38" value="${familyID}" autocomplete="off" class="input form-control"/>
                <input type="hidden" name="familyID" id="familyID" value="${familyID}"/>
                <span class="input-group-btn">
                <button type="button" class="btn btn-primary" disabled
                        onClick="$('#familyID').val($('#familyCode').val());submit()" ><i class="glyphicon glyphicon-stats"></i> 进入变更购房人页面</button>
                </span>
            </div>
        </center>
    </div>
...
<script>
    $(function(){
        var options = {
            serviceUrl: '/cq/chooseHouse/ajaxFamilyInfo',
    ...
    function submit() {
        ...
        $.get('/cq/chooseHouse/validateIsExists', {familyCode: familyCode, _time: new Date().getTime()}, function (data) {
            if(data.status=="1"){
                location.href = "form?familyId=" + data.familyId;
            }else{
                alert(data.message);
            }
        });
