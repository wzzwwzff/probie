...
public class FamilyService extends HibernateDao<Family>{
     ...
     * @return
     */
    public List<String> getGroupByBd(String bdCode) {
        String hql = "SELECT DISTINCT groupCode FROM Family where bdCode =" + bdCode;
        List<String> list = this.getSession().createSQLQuery(hql).list();
        return list;
    }
