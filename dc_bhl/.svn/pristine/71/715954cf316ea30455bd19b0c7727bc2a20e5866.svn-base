...
//    private Integer armyNum;//现役军人
    ...
    private BigDecimal bcTotalMoney;  //补偿款总计（补助，补偿各种加项）

    @Column(columnDefinition = "INT default 1", nullable = false, insertable = false)
    private Integer sureStatus;   //数据锁定状态(拆迁) 1:未锁定，2：已锁定

    @Column(columnDefinition = "INT default 1", nullable = false, insertable = false)
    private Integer surePgStatus;   //数据锁定状态(评估) 1:未锁定，2：已锁定

    @Column(columnDefinition = "INT default 1", nullable = false, insertable = false)
    private Integer sureChStatus;   //数据锁定状态(测绘) 1:未锁定，2：已锁定
    ...
    public Integer getSurePgStatus() {
        return surePgStatus;
    }

    public void setSurePgStatus(Integer surePgStatus) {
        this.surePgStatus = surePgStatus;
    }

    public Integer getSureChStatus() {
        return sureChStatus;
    }

    public void setSureChStatus(Integer sureChStatus) {
        this.sureChStatus = sureChStatus;
    }
    ...
    public BigDecimal getBcTotalMoney() {
        return bcTotalMoney;
    }

    public void setBcTotalMoney(BigDecimal bcTotalMoney) {
        this.bcTotalMoney = bcTotalMoney;
    }
