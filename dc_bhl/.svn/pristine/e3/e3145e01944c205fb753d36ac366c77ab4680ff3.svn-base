            ...
            <c:forEach items="${pageInfo.result}" var="family" varStatus="status">
                <tr>
                    ...
                    <td class="text-center">
                        <a href="javascript:cancelFile('${family.id}')" title="撤销图片"><i
                                class="fa fa-edit"></i>&nbsp;撤销图片
                        </a>
                    </td>
                </tr>
            </c:forEach>
...
<script>
    ...
    function cancelFile(familyId) {
        if (confirm("确定要将该家庭上传照片撤销吗？")) {
            $.get("cancelFile", {familyId: familyId, _date: new Date().getTime()}, function (data) {
