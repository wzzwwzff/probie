...
<div class="content">
    ...
    <fieldset>
        <legend>三、可申请购买房源</legend>
        <table class="table table-bordered table-striped">
            <tbody>
            <c:forEach items="${projectList}" var="project">
                <tr>
                    <td class="text-right" style="width: 240px;;font-size:24px; ">${project.projectName}</td>
                    <td style="line-height: 35px;text-align: left;">
                        <c:forEach items="${projectBuildNumMap[project.id]}" var="buildNum" varStatus="status">
                            <c:if test="${status.index ne 0 && status.index % 6 eq 0}"><br/></c:if>
                            <button type="button" projectUuid="${project.id}" buildNum="${buildNum}" projectName="${project.projectName}" class="btn btn-primary buildNumBtn" style="width: 150px;">
        ...
        <div style="width: 100%;text-align: center;font-weight: bold;font-size: 22px;color: black;margin-top: 20px;" id="buildInfoTitle"></div>
...
<script>
    ...
    function delHouseInfo(houseId) {
        if (confirm("确定删除吗？")) {
            $.get("delHouseInfo", {houseId: houseId,familyId:"${family.id}", _time: new Date().getTime()}, function (data) {
                if (data.code == "0000"){
                    loadSelectHouseInfo();
                }else {
                    alert(data.message);
    ...
    function changeHouseInfo(houseId) {
        if (confirm("确定平移房源信息吗？")) {
            $.get("changeHouseInfo", {houseId: houseId,familyId:"${family.id}", _time: new Date().getTime()}, function (data) {
                if (data.code == "0000"){
                    loadSelectHouseInfo();
                }else {
                    alert(data.message);
    ...
    //改选房源，撤销选房状态
    function cancelXfStatus() {
        $.get("cancelXfStatus", {familyId: "${family.id}", _time: new Date().getTime()}, function (data) {
            if (data.code == "0000") {
                loadSelectHouseInfo();
            } else {
                alert(data.message);
