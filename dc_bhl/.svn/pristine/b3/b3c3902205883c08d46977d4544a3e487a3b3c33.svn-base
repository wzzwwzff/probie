...
//                    response.reset();
    ...
    @ResponseBody
    public Map editSave(HttpServletRequest request) {
        ...
        for (String s : idArray) {
            ...
            if(!name.equals("")&&!idCard.equals("")){
                name=name+","+familyPerson.getName();
                idCard=idCard+","+familyPerson.getIdCard();
            }else{
                name=name+familyPerson.getName();
                idCard=idCard+familyPerson.getIdCard();
            }
        ...
        OperationInfo operationInfo = new OperationInfo();
        operationInfo.setOperateDate(new Date());
        operationInfo.setOperatePerson(GetCurrentUser.getLoginUser(request).getRealName());
        operationInfo.setFamilyId(familyId);
        operationInfo.setMemo("修改购房人，从："+house.gethPerson()+" 修改为："+name);
        operationInfoService.save(operationInfo);
    ...
    public Response saveBuildStatus(HttpServletRequest request){
        Integer houseId = ParamUtils.getInt(request, "houseId", 0);//houseId
        String familyCode = ParamUtils.getString(request, "familyCode", "");//家庭id
        House house = this.houseService.get(houseId);
        Precondition.checkAjaxArguement(house != null,"1111","数据错误！");
        Family family = familyService.getFamilyByFamilyCode(familyCode);
        Precondition.checkAjaxArguement(family != null,"2222","数据错误！");
        Servlets.bind(request, house);
        house.setFamily(family);
        house.setChooseDate(new Date());
    ...
    public String save(HttpServletRequest request) {
        ...
        return "redirect:editHPerson?familyId=" + familyId +"&houseId="+houseId;
    }
}
