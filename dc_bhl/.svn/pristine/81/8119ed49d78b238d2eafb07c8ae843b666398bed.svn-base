...
bootstrapQ.confirm = function (options, ok, cancel) {
    ...
    opt.btn = true;
    if (opt.id == undefined || opt.id == null || opt.id == "") {
        opt.id = "confirm" + new Date().getTime();
    }
    // append
    $('body').append(bootstrapQ.modalstr(opt));
...
bootstrapQ.dialog = function (options, func) {
    ...
    opt.big = true;
    if (opt.id == undefined || opt.id == null || opt.id == "") {
        opt.id = "dialog" + new Date().getTime();
    }
    // append
    $('body').append(bootstrapQ.modalstr(opt));
...
bootstrapQ.msgstr = function (msg, type, position) {
    ...
    return '<div class="alert alert-' + type + ' alert-dismissible" role="alert" style="display:none;position:fixed;' + position + ':0;left:0;width:100%;z-index:2001;margin:0;text-align:center;" id="bsalert' + times + '"><button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>' + msg + '</div>';
};
bootstrapQ.msg = function (options) {
    ...
    $('.alert-dismissible[id^="bsalert"]').slideDown();
    setTimeout(function () {
        $('.alert-dismissible[id^="bsalert"]').slideUp(function () {
            $(this).remove();
        });
    }, opt.time);
};
