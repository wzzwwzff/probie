...
<table id="myData" border="1" cellspacing="4" cellpadding="4" style="width: 98%;margin-top: 0;">
    <tr>
        ...
        <td style="width: 20%;">补偿安置意向书序号</td>
    ...
    <c:forEach items="${pageInfo.result}" var="family">
        <tr>
            ...
            <td style="color: red;"> <c:if test="${family.signNum == 0}">/</c:if>
                <c:if test="${family.signNum != 0}"><fmt:formatNumber value="${family.signNum}" pattern="000"/></c:if></td>
            <td style="color: lawngreen;"> <c:if test="${family.signNum == 0}">/</c:if>
                <c:if test="${family.signNum != 0}"><tags:longToDate value="${family.signYxDateLong}" /></c:if></td>
