...
<div class="title"><h4>拆迁数据确认页面【${family.name}】</h4></div>
<div class="content">
    <%@include file="/WEB-INF/pages/cq/family/common/view/rhView.jsp" %><br/><br/>
    <div class="footer">
        <c:set var="noCheck" value="<%=CheckState.NOCHECK.getIndex()%>"/>
        <c:set var="firstBack" value="<%=CheckState.FIRSTBACK.getIndex()%>"/>
        <button type="button" class="btn btn-lg btn-primary"
                onclick="sureData('${family.id}')" ${ family.sureStatus eq 1  ? '':'disabled'}>
...
<script>
    function sureData(familyId) {
        if (confirm("确认数据锁定吗？\n锁定后数据将不可修改！")) {
            $.get("sureData", {familyId: familyId,type:"cc"}, function (data) {
