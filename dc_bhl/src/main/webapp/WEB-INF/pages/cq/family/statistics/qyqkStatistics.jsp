...
<div class="content">
    ...
    <table class="table table-bordered table-hover table-striped">
        ...
        <thead>
        <tr>
            <th class="text-center" rowspan="3" style="vertical-align: middle;width: 11%">分指</th>
        ...
        <c:forEach items="${dataDictList1}" var="dataDict1" >
            ...
            <tr>
                <td><a href="/cq/family/statistics/qyqkGroupStatistics?bdCode=${dataDict1.attributeValue}&date=${date}">${dataDict1.attributeName}</a></td>
                    <%--求总户数的循环--%>
                ...
                <c:set var="itemAllCount" value="${itemAllCount + valueMap['0']['allCount']}" />
                <td>${empty itemAllCount ? 0 : itemAllCount}</td>
                    <%--输出值的循环--%>
                <c:forEach var="dataDict" items="${dataDictList2}">
                    <c:set var="value" value="${valueMap[dataDict.attributeValue]}"/>
                    <td>${empty value["todayQyCount"] ? 0 : value["todayQyCount"]}</td>
                </c:forEach>
                <td>${empty itemAllTodayCount ? 0 : itemAllTodayCount}</td>
                <c:forEach var="dataDict" items="${dataDictList2}">
                    <c:set var="value" value="${valueMap[dataDict.attributeValue]}"/>
                    <td>${empty value["qyCount"] ? 0 : value["qyCount"]}</td>
                </c:forEach>
                <td>${empty itemAllQyCount ? 0 : itemAllQyCount}</td>
                <td>
                    <c:if test="${itemAllCount gt 0}">
                        <%--<c:out value="${itemAllQyCount}/" />--%>
                        <fmt:formatNumber  maxFractionDigits="2" value="${itemAllQyCount/itemAllCount}" type="percent" />
                    </c:if>
                </td>
            </tr>
        </c:forEach>
        <tr>
            ...
            <c:set var="allCount" value="${allCount + allMap['0']['allCount']}" />
            <td>合计</td>
            <td>${allCount}</td>
            <c:forEach var="dataDict" items="${dataDictList2}">
            ...
            <td>
                <c:if test="${allCount gt 0}">
                    <fmt:formatNumber maxFractionDigits="2" value="${allQyCount/allCount}"  type="percent" />
                </c:if>
            </td>
        </tr>
