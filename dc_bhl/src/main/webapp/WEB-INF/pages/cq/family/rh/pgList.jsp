    ...
    <table class="table table-bordered table-hover">
        ...
        <c:forEach items="${pageInfo.result}" var="family" varStatus="status">
            <tr>
                ...
                <td class="text-left"><tags:dataDict dataDict="锁定状态" value="${family.surePgStatus}"/></td>
                ...
                <td class="text-center">
                    ...
                    <c:if test="${(family.firstCheckStatus eq 1 and family.surePgStatus eq 1) or (family.firstCheckStatus eq 1 and family.surePgStatus eq 2) or family.firstCheckStatus eq 0 }">
                        <a href="pgForm?familyId=${family.id}&${pageInfo.searchParams}"><i class="fa fa-big fa-edit"></i>&nbsp;录入</a>&nbsp;&nbsp;
                    </c:if>
                    <c:if test="${family.firstCheckStatus eq 1 or family.surePgStatus eq 1}">
                        <a href="/cq/family/rh/dataSurePgList?familyId=${family.id}"><i class="fa fa-big fa-eye"></i>&nbsp;数据锁定</a>&nbsp;&nbsp;
                    </c:if>
                    <c:if test="${family.surePgStatus eq 2 and (family.firstCheckStatus eq 1 or family.firstCheckStatus eq 0)}">
...
<script>
    function cancleData(familyId) {
        if (confirm("确认评估数据撤销锁定吗？\n撤销锁定后评估数据将可以修改！")) {
            $.get("cancleData", {familyId: familyId,type:"pg"}, function (data) {
