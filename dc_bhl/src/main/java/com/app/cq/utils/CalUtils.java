...
public class CalUtils {
    ...
    private static FamilyPersonService familyPersonService;
    ...
    public synchronized static void calc(Family family,String typeAccount) {
        ...
        familyPersonService = (FamilyPersonService) webApplicationContext.getBean(FamilyPersonService.class);
        houseService = (HouseService) webApplicationContext.getBean(HouseService.class);
...
//    }
     ...
     * @param family
     */
    private static void calGfMoney(Family family) {
        //从数据字典中取出各移机的单价费用
        BigDecimal fgcbMoney = new BigDecimal(bzMap.get("房改成本价格"));
        BigDecimal jjsyMoney = new BigDecimal(bzMap.get("经济适用住房价格"));

        List<FamilyPerson> familyPersonList = familyPersonService.getPersonListByFamilyId(family.getId(),1);
        List<House> houseList = houseService.getHouseListByFamilyId(family.getId());
        int familyPersonSize = familyPersonList.size() == 0 ? 1 : familyPersonList.size();

        BigDecimal houseAreaTotal = new BigDecimal(0);//选房总面积
        for (int i = 0; i <houseList.size() ; i++) {
            houseAreaTotal = BigDecimalUtils.getSum(houseAreaTotal,houseList.get(i).getBuildArea());
        }

        BigDecimal helpMath = BigDecimalUtils.getBigAndBigSub(new BigDecimal(1), BigDecimalUtils.getBigAndIntMul(new BigDecimal(0.05), family.getHelpNum() == null ? 0 : family.getHelpNum()));//援藏系数
        BigDecimal teacherMath = BigDecimalUtils.getBigAndBigSub(new BigDecimal(1), BigDecimalUtils.getBigAndIntMul(new BigDecimal(0.05), family.getTeacherNum() == null ? 0 : family.getTeacherNum()));//教师系数
        BigDecimal glMath = BigDecimalUtils.getBigAndBigSub(new BigDecimal(1), BigDecimalUtils.getBigAndIntMul(new BigDecimal(0.009), family.getGlNum() == null ? 0 : family.getGlNum()));//夫妻工龄之和系数

        BigDecimal pjFifArea = BigDecimalUtils.getBigAndIntMul(new BigDecimal(15), familyPersonSize);//人均15的面积
        BigDecimal yAreaMoney = new BigDecimal(0);//未超出原建筑面积部分购房款
        BigDecimal wcArea = new BigDecimal(0);//超出原建筑面积但未超过人均15 ㎡部分面积
        BigDecimal wcAreaMoney = new BigDecimal(0);//超出原建筑面积但未超过人均15 ㎡部分购房款
        BigDecimal ccArea = new BigDecimal(0);//超过人均15 ㎡部分面积
        BigDecimal ccAreaMoney = new BigDecimal(0);//超过人均15 ㎡部分购房款

        BigDecimal big1 = BigDecimalUtils.getBigAndBigMul(BigDecimalUtils.getBigAndBigMul(helpMath, teacherMath),glMath);//系数之积
        BigDecimal yMoney = BigDecimalUtils.getBigAndBigMul(BigDecimalUtils.getBigAndBigMul(big1,new BigDecimal(0.94)),fgcbMoney);//1560*94%*（1-0.9%*夫妻工龄之和）*（1-5%*教师资格证数）*（1-5%*援藏人数））


        yAreaMoney = BigDecimalUtils.getBigAndBigMul(yMoney,family.getBuildArea());
        if (houseAreaTotal.compareTo(family.getBuildArea()) > 0){//超过原住房面积
            if (houseAreaTotal.compareTo(pjFifArea) > 0){//且超过人均15 ㎡部分
                ccArea = BigDecimalUtils.getBigAndBigSub(houseAreaTotal,pjFifArea);
                wcArea = BigDecimalUtils.getBigAndBigSub(BigDecimalUtils.getBigAndBigSub(houseAreaTotal,family.getBuildArea()),ccArea);
                ccAreaMoney = BigDecimalUtils.getBigAndBigMul(ccArea, jjsyMoney);
                wcAreaMoney = BigDecimalUtils.getBigAndBigMul(wcArea, fgcbMoney);
            }else if (houseAreaTotal.compareTo(pjFifArea) <= 0){//未超过人均15 ㎡部分
                wcArea = BigDecimalUtils.getBigAndBigSub(houseAreaTotal,family.getBuildArea());
                wcAreaMoney = BigDecimalUtils.getBigAndBigMul(wcArea, fgcbMoney);
            }
        }

        BigDecimal totalAreaMoney = BigDecimalUtils.getSum(yAreaMoney, wcAreaMoney, ccAreaMoney);//总购房款

        family.setHelpYhMoney(helpMath);
        family.setTeacherYhMoney(teacherMath);
        family.setYjzMoney(yAreaMoney);
        family.setWcjzArea(wcArea);
        family.setWcjzMoney(wcAreaMoney);
        family.setCcjzArea(ccArea);
        family.setCcjzMoney(ccAreaMoney);
        family.setGfTotalMoney(totalAreaMoney);
    }
     ...
     * @param family
     */
    public static void calBcTotalMoney(Family family) {
        BigDecimal bqMoney = family.getBqMoney();
        BigDecimal zhknMoney = family.getZhknMoney();
        BigDecimal ydjlfyMoney = family.getYdjlfyMoney();
        BigDecimal hbbcMoney = family.getHbbcMoney();
        BigDecimal sfpfFee = family.getSfpfFee();
        BigDecimal qysdFee = family.getQysdFee();
        BigDecimal gcphMoney = family.getGcphMoney();
        BigDecimal shknFee = family.getShknFee();
        BigDecimal yjMoney = family.getYjMoney();

        BigDecimal total = BigDecimalUtils.getSum(bqMoney, zhknMoney, ydjlfyMoney, hbbcMoney, sfpfFee, qysdFee, gcphMoney, shknFee, yjMoney);
        family.setBcTotalMoney(total);
    }
