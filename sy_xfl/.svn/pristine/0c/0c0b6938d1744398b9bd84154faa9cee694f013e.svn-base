        ...
        <table class="table table-bordered table-hover table-striped">
            ...
            <c:forEach items="${pageInfo.result}" var="family" varStatus="status">
                <tr>
                    ...
                    <td class="text-center">
                        <c:if test="${family.jfNumber eq maxNumber}">
                        <a href="javaScript:cancelNumber('${family.id}')"
                           title="撤销"><i class="fa fa-close"></i>撤销交房顺序号
                        </a>
                        </c:if>
                    </td>
                </tr>
...
<script>
    function cancelNumber(familyId) {
        if (confirm("确认撤销选房顺序号吗，撤销后交选房状态将改为未交房？")) {
            $.get("cancelNumber", {familyId: familyId, myTime: new Date().getTime()}, function (data) {
                if (data.code == "0000") {
                    location.reload();
                } else {
                    alert(data.message);
