...
public class FamilyPersonService  extends HibernateDao<FamilyPerson>{
     ...
     * @return
     */
    public List<FamilyPerson> getAzFamilyPerson(Integer familyId){
        String hql = "from FamilyPerson where family.id=? and status = 2 order by id";
        return list(hql, familyId);
    }
    ...
    public List<FamilyPerson> listNoPerson(Integer familyId) {
        return this.list("from FamilyPerson where familyId = " + familyId + "  and marrType=4");
    }
    //得到家庭成员数量
    ...
    //得到家庭成员数量及身份证号
    public String getPersonStrNum(List<FamilyPerson> familyPersonList){
        String personStr = "";
        if (familyPersonList != null && familyPersonList.size() > 0) {
            for(FamilyPerson familyPerson:familyPersonList){
                personStr = personStr+familyPerson.getPersonName()+"(" + familyPerson.getIdCard() + "),";
            }
        }
        if (personStr.length()>0){
            personStr = personStr.substring(0,personStr.length()-1);
