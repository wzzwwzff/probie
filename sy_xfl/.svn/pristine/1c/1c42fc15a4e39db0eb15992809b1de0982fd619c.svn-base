...
<div class="title"><h4>复核页面【${family.name}，<font color="red">当前状态：</font><font color="green"><tags:dataDict
        dataDict="上报状态" value="${family.chState}"/></font>】</h4></div>
<div class="content">
    <%@include file="/WEB-INF/pages/cq/common/view/chView.jsp" %>
    <div class="footer" style="text-align: center">
        <button class="btn btn-success btn-lg" <c:if test="${family.chState eq 2}"> disabled </c:if>
    ...
    function handUp(familyId) {
        ...
        if (confirm("确定上报测绘数据吗？")) {
            $.get("handUp", {
                ...
                if (data.code == "0000") {
                    window.location.href = "chCheckList?<tags:searchParams/>";
                } else {
                    alert(data.message);
