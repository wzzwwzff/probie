...
//        List<DataDict> dataDictList = this.dataDictService.getDataDictList("组别");
    ...
    //选房顺序号展示正序
    @RequestMapping("xfNumberShowUp")
    public void xfNumberShowUp(){
     ...
     * @return
     */
    @RequestMapping("getXFCount")
    @ResponseBody
    public Map<String, Integer> sequenceXF(HttpServletRequest request) {
        Integer xfCount = statisticsService.getXFCount();
        Map<String, Integer> map = new HashMap<String, Integer>();
     ...
     * @param modelMap
     */
    @RequestMapping("xfNumberShowItemUp")
    public void xfNumberShowItemUp(HttpServletRequest request,ModelMap modelMap) {
        int myPageNo = ParamUtils.getInt(request, "myPageNo", 1);
        int myPageSize = ParamUtils.getInt(request, "myPageSize", 10);
        ...
        pageInfo = this.statisticsService.xfNumberShowUp(pageInfo);
        modelMap.addAttribute("pageInfo", pageInfo);
    }
}
