...
<div class="content">
    <form action="save?<tags:searchParams/>" method="post">
        ...
        <table class="table table-bordered table-hover">
            ...
            %>
            <tr>
                <td class="text-center"><input type="text" name="attributeName<%=i%>" value="<%=name%>"/></td>
                <td class="text-center"><input type="text" class="inputColor" name="attributeColor<%=i%>" value="<%=color%>"/></td>
                <td class="text-center"><input type="text" name="attributeValue<%=i%>" value="<%=value%>"/></td>
                <td class="text-center"><input name="validFlag<%=i%>" type="checkbox" value="1" <%
                    if (valid == 1) {
                        out.println("checked");
                    }
                %>/></td>
                <td class="text-center"><input type="text" name="orderId<%=i%>" value="<%=orderId%>" size="6"/></td>
            </tr>
            <%}%>
            </tbody>
        </table>
