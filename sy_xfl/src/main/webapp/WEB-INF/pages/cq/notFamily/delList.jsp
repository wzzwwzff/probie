        ...
        <c:forEach items="${pageInfo.result}" var="notFamily" varStatus="status">
            <tr>
                ...
                <td class="text-center">
                    ...
                    <a href='/cq/notFamily/delForm?notFamilyId=${notFamily.id}&<tags:searchParams></tags:searchParams>' title="修改"><i class="fa fa-edit"></i>&nbsp;修改</a>&nbsp;&nbsp;
                    <a href="#" onclick="backDelete(${notFamily.id})"><i class="fa fa-times-circle"></i>&nbsp;撤销删除状态</a>
                </td>
            </tr>
        </c:forEach>
    </table>
...
<script>
    // 数据上报
    function backDelete(notFamilyId) {
        if (confirm("确定撤销删除数据吗？")) {
            $.get("backDelNotFamily", {
                notFamilyId: notFamilyId,
                _date: new Date().getTime()
            }, function (data) {
                if (data.code == "0000") {
                    window.location.href = "delList?<tags:searchParams/>";
                } else {
                    alert(data.message);
