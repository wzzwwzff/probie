                ...
                <tr>
                    ...
                    <td class="text-center">
                        ...
                        <c:if test="${family.pgState < 2}">
                            <a href='pgCheck?familyId=${family.id}&
                                <tags:searchParams></tags:searchParams>'
                               title="复核上报"><i class="fa fa-edit"></i>&nbsp;复核上报
                            </a>
                        </c:if>
                        <c:if test="${family.pgState eq 2 }">
...
<script>
    function cancelCheck(familyId) {
        if (confirm("确认撤销评估数据复核上报吗？\n撤销后评估数据将可以该修改！")) {
            $.get("cancelCheck", {type: 'pg', familyId: familyId, myTime: new Date().getTime()}, function (data) {
                if (data.code == "0000") {
                    location.reload();
                } else {
                    alert(data.message);
